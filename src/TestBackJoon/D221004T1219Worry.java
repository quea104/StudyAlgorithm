...
public class D221004T1219Worry {
	...
	public static void main(String[] args) throws IOException {
		...
		String ans = null;
		if(dist[E] == MIN)
			ans = "gg"; // 도착 도시에 도착하는 것이 불가능
		else {
			if(cycleEdge.size() != 0) {
				boolean[] visited = bfs(cycleEdge); 
				
				if(visited[E])
					ans = "Gee"; // 순환 간선이 도착 정점에 도착하므로 번 돈이 무한대
				else
					ans = Long.toString(dist[E]); // 순환 간선이 도착 정점에 가지는 않으나 이미 계산된 이윤값이 정답
			}
			else {
	...
	private static ArrayList<Integer> search() {
		...
		for(int i = 1; i <= N; i++) {
			for(int j = 0; j < list.size(); j++) {
				...
				if(dist[x] == MIN) // x가 한번도 방문하지 않은 곳이면 체크할 필요 없음. 아직 단절된 상태 
					continue;
				else if(dist[y] < dist[x] + w + earn[y]) {
					dist[y] = dist[x] + w + earn[y];
					
					// 양의 간선 순환이 발생한 정점 변수에 따로 저장
					if(i == N) {
						cycleEdge.add(y);
					}
				}
	...
	private static boolean[] bfs(ArrayList<Integer> cycleEdge) {
		...
		// 양의 간선 순환과 연관된 정점이 도착 정점으로 갈 수 있는지 확인
		for(int s: cycleEdge) {
			if(!visited[s]) {
